...
(test query-decode
  ...
  (is (equal (m2cl::query-parse "foo=") '(("foo" . nil))))
  (is (equal (m2cl::query-parse "foo=bar") '(("foo" . "bar"))))
  (is (equal (m2cl::query-parse "foo=&foo2=baz") '(("foo" . nil)
                                                   ("foo2" . "baz"))))
  (is (equal (m2cl::query-parse "foo=;foo2=baz") '(("foo" . nil)
                                                   ("foo2" . "baz"))))
  (is (equal (m2cl::query-parse "foo=bar&foo2=baz") '(("foo" . "bar")
                                                      ("foo2" . "baz"))))
